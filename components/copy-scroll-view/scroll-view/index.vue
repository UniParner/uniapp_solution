...
<script>
  ...
  import disableScrollBounce from '../uni-copy-share/scroll.js'
  import supportsPassive from '../uni-copy-share/env.js'
  ...
  export default {
    ...
    mounted() {
      ...
      this.__handleTouchMove = function(event) {
          ...
          if (Math.abs(x - touchStart.x) > Math.abs(y - touchStart.y)) {
            ...
            if (self.scrollX) {
              if (main.scrollLeft === 0 && x > touchStart.x) {
                needStop = false
                return
              } else if (main.scrollWidth === main.offsetWidth + main.scrollLeft &&
                x < touchStart.x) {
                needStop = false
                return
              }
            ...
            if (self.scrollY) {
              if (main.scrollTop === 0 && y > touchStart.y) {
                needStop = false
                return
              } else if (main.scrollHeight === main.offsetHeight + main.scrollTop &&
                y < touchStart.y) {
                needStop = false
                return
              }
        ...
        if (self.refresherEnabled && self.refreshState === 'pulling') {
          ...
          //   deltaY: dy
          // })
          self.$emit('refresherpulling', event, { deltaY: dy })
        }
      }
      ...
      this.__handleTouchEnd = function(event) {
        ...
        if (self.refresherHeight >= self.refresherThreshold) {
          ...
          self.$emit('refresherabort', event, {})
        }
      }
      this.$on('touchstart', () => {
        console.log('开始触摸了：..')
      })
      this.$refs.main.addEventListener('touchstart', this.__handleTouchStart,
        passiveOptions)
      ...
      this.$refs.main.addEventListener('touchend', this.__handleTouchEnd,
        passiveOptions)
    },
    activated() {
      // 还原 scroll-view 滚动位置
    ...
    beforeDestroy() {
      this.$refs.main.removeEventListener('touchstart', this.__handleTouchStart,
        passiveOptions)
      this.$refs.main.removeEventListener('touchmove', this.__handleTouchMove,
        passiveOptions)
      this.$refs.main.removeEventListener('scroll', this.__handleScroll,
        supportsPassive ? {
          passive: false
        } : false)
      this.$refs.main.removeEventListener('touchend', this.__handleTouchEnd,
        passiveOptions)
    },
    methods: {
      ...
      scrollTo: function(t, n) {
        ...
        t < 0 ? t = 0 : n === 'x' && t > i.scrollWidth - i.offsetWidth ? t =
          i.scrollWidth - i.offsetWidth :
          n === 'y' && t > i.scrollHeight - i.offsetHeight && (t = i.scrollHeight -
            i.offsetHeight)
        var r = 0
        var o = ''
      ...
      _handleTrack: function($event) {
        ...
        if (this._noBubble === null && this.scrollY) {
          if (Math.abs(this._y - $event.detail.y) / Math.abs(this._x - $event
              .detail.x) > 1) {
            this._noBubble = true
          } else {
            this._noBubble = false
        ...
        if (this._noBubble === null && this.scrollX) {
          if (Math.abs(this._x - $event.detail.x) / Math.abs(this._y - $event
              .detail.y) > 1) {
            this._noBubble = true
          } else {
            this._noBubble = false
      ...
      _handleScroll: function($event) {
        if (!($event.timeStamp - this._lastScrollTime < 20)) {
          ...
          if (this.scrollY) {
            if (target.scrollTop <= this.upperThresholdNumber && this.lastScrollTop -
              target.scrollTop > 0 && $event.timeStamp - this.lastScrollToUpperTime >
              200) {
              // this.$trigger('scrolltoupper', $event, {
              //   direction: 'top'
              // })
              this.$emit('scrolltoupper', $event, { direction: 'top' })
              this.lastScrollToUpperTime = $event.timeStamp
            }
            if (target.scrollTop + target.offsetHeight + this.lowerThresholdNumber >=
              target.scrollHeight && this.lastScrollTop - target.scrollTop <
              0 && $event.timeStamp - this.lastScrollToLowerTime > 200) {
              // this.$trigger('scrolltolower', $event, {
              //   direction: 'bottom'
              // })
              this.$emit('scrolltolower', $event, { direction: 'bottom' })
              this.lastScrollToLowerTime = $event.timeStamp
            }
          }
          if (this.scrollX) {
            if (target.scrollLeft <= this.upperThresholdNumber && this.lastScrollLeft -
              target.scrollLeft > 0 && $event.timeStamp - this.lastScrollToUpperTime >
              200) {
              // this.$trigger('scrolltoupper', $event, {
              //   direction: 'left'
              // })
              this.$emit('scrolltoupper', $event, { direction: 'left' })
              this.lastScrollToUpperTime = $event.timeStamp
            }
            if (target.scrollLeft + target.offsetWidth + this.lowerThresholdNumber >=
              target.scrollWidth && this.lastScrollLeft - target.scrollLeft <
              0 && $event.timeStamp - this.lastScrollToLowerTime > 200) {
              // this.$trigger('scrolltolower', $event, {
              //   direction: 'right'
              // })
              this.$emit('scrolltolower', $event, { direction: 'right' })
              this.lastScrollToLowerTime = $event.timeStamp
            }
          }
      ...
      _mainRect: async function() {
        return new Promise((resolve) => {
          uni.createSelectorQuery().in(this).select('.main').boundingClientRect(
            rect => { resolve(rect) }).exec()
        })
      },
      _scrollIntoViewChanged: async function(val) {
        if (!val) { return }
        ...
        console.log('祛火去。。。')
        const elRect = await this.getIntoViewRect(val)
        const mainRect = await this._mainRect()
        // const elRect = await this._intoViewBoundingClientRect(val)
        if (!elRect || !mainRect) {
          console.log('获取失败')
          return
        }
        // var mainRect = this.$refs.main.getBoundingClientRect()
        console.log('获取的elRect信息：', JSON.stringify(elRect), JSON.stringify(mainRect))
        if (this.scrollX) {
      ...
      _setRefreshState(state) {
        switch (state) {
          case 'refreshing':
            ...
            this.$emit('refresherrefresh', event, {})
            break
          case 'restore':
            this.refresherHeight = 0
            // this.$trigger('refresherrestore', {}, {})
            this.$emit('refresherrestore', event, {})
            break
        }
